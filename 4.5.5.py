...
def calculate_total_price(html: str) -> float:
    ...
    # Ваш код
    list_of_prices = [float(i.text[7:]) for i in soup.select('p.price')]
    list_of_amount = [int(i.text[22:]) for i in soup.select('p.stock')]
    return sum(map(lambda x, y: x * y, list_of_prices, list_of_amount))
